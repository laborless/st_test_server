<template>
  <h3>📥UDP Echo server packets📤</h3>
  <div v-if="$root.session == 0">
    <a><strong>Please Sign In</strong></a>
  </div>
  <div v-else>
    <p style="text-align: right; margin-right: 20px;"><a href="https://193.122.103.95/api/udpecho/db" download><strong>Save DB💾</strong></a></p>
...
export default {
  ...
  methods: {
    ...
    async loadPage (num) {
      try {
        const response = await fetch('https://' + window.location.hostname + '/api/udpecho/page/' + num, {
          // mode: 'cors',
          credentials: 'include'
        })
        if (response.status === 200) {
